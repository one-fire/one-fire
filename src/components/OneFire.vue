<template>
  <div>
    ...
    <grid :cols="2" :show-lr-borders="false" :show-vertical-dividers="false">
      <grid-item >
        <x-button type="warn" class="btn" data-clipboard-target="#output-content" text="加密" @click.native="encrypt"/>
      </grid-item>
      <grid-item>
        <x-button type="primary" class="btn" data-clipboard-target="#output-content" text="解密" @click.native="decrypt"/>
      </grid-item>
    </grid>
...
export default {
  ...
  created () {
    let clipboardOut = new Clipboard('.btn')
